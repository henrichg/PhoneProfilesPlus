    ...
    String getPreferencesDescription(boolean addBullet, boolean addPassStatus, Context context)
    {
        ...
        if (!this._enabled) {
            ...
            if (Event.isEventPreferenceAllowed(PREF_EVENT_ORIENTATION_ENABLED, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) {
                ...
                } else if (extenderVersion < PPApplication.VERSION_CODE_EXTENDER_6_2) {
                    selectedApplications = context.getString(R.string.profile_preferences_device_not_allowed) +
...
//                Log.e("EventPreferencesOrientation.setSummary", "checkLight = false");
        ...
        if (key.equals(PREF_EVENT_ORIENTATION_INSTALL_EXTENDER)) {
            ...
            if (preference != null) {
                ...
                if (extenderVersion == 0) {
                    ...
                    if (extenderVersion < PPApplication.VERSION_CODE_EXTENDER_6_2)
    ...
    void checkPreferences(PreferenceManager prefMng, Context context) {
        ...
        enabled = PPPExtenderBroadcastReceiver.isEnabled(context.getApplicationContext(), PPApplication.VERSION_CODE_EXTENDER_6_2);
                    ...
                    synchronized (PPApplication.orientationScannerMutex) {
                        if ((PhoneProfilesService.getInstance() != null) && PhoneProfilesService.getInstance().isOrientationScannerStarted()) {
                            ...
                            if (!_ignoredApplications.isEmpty()) {
                                if (PPPExtenderBroadcastReceiver.isEnabled(eventsHandler.context.getApplicationContext(), PPApplication.VERSION_CODE_EXTENDER_6_2)) {
