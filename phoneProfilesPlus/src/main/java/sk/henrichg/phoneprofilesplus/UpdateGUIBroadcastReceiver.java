...
public class UpdateGUIBroadcastReceiver extends BroadcastReceiver {
            ...
            try {
                if (PPApplication.getApplicationStarted(true)) {
                    ...
                    if (workManager != null) {
                        ...
                        //}
                        //workManager.enqueueUniqueWork("elapsedAlarmsUpdateGUIWork", ExistingWorkPolicy.KEEP, worker);
    ...
    private static void _doWork(/*boolean useHandler,*/ Context context, final boolean refresh, final boolean alsoEditor/*, final boolean fromAlarm*/) {
        ...
        if (!refresh) {
            if (PPApplication.lockRefresh || EditorProfilesActivity.doImport)
                // no refresh widgets
                return;
        }
        ...
        if (refresh || (now - PPApplication.lastRefreshOfGUI) >= PPApplication.DURATION_FOR_GUI_REFRESH) {
            ...
            PPApplication.forceUpdateGUI(context.getApplicationContext(), alsoEditor, true, refresh);
        } else {
